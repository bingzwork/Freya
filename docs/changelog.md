...
## Unreleased - Phase 2: TaskGraph Runtime Integration

**Planning & Reasoning Phase 2** wires the existing `TaskGraph` module into the FreyaAgent runtime pipeline, replacing flat step lists with a proper dependency DAG.

- **Planner (`app/agent/planner.py`)**
  - `create_plan()` now builds a `TaskGraph` with sequential dependencies: step N+1 depends on step N
  - Each planning step becomes a `TaskNode` with parent/child relationships via `DependencyEdge`
  - Returns a fully-populated `Plan` object with validated graph

- **PlanManager (`app/planner/plan_manager.py`)**
  - `_rebuild_schedule()` preserves task dependencies by reading from `Task.dependencies` field
  - `add_dependency()` updates both the graph and the task's `.dependencies` list for persistence
  - Cycle detection via `CycleDetectedError` propagates to caller

- **TaskGraph (`app/planner/task_graph.py`)**
  - `add_dependency()` / `remove_dependency()` now maintain `TaskNode.children` / `TaskNode.parents`
  - Topological sort validates acyclicity; rejects cycles with `CycleDetectedError`

- **Executor (`app/agent/executor.py`)**
  - `execute_plan()` consumes `Plan` objects and uses `TaskGraph.topological_sort()` for execution order
  - Dict-based plans remain supported for backward compatibility
  - Cyclic dependency rejection surfaces as `ValueError` at execution time

- **Tests (`tests/test_planner_agent.py`)**
  - 6 new tests covering TaskGraph integration: node creation, dependency edges, parent/child relationships, cycle detection, topological execution order, completed-task preservation

- **Behaviour:** Plans generated by `Planner` are now DAGs with live dependency tracking; execution follows topological order; completed tasks remain in the graph for future replanning.
