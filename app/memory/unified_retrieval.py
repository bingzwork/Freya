...
class LongTermMemoryRetriever(MemoryRetriever):
    ...
    def retrieve(self, query: RetrievalQuery) -> List[RetrievalResult]:
        ...
        try:
            ...
            entries = self.memory.search(query.query, limit=query.max_results)

            for entry in entries:
                # Boost preference/fact categories for relevance
                category_boost = 0.1 if entry.category in ("preference", "fact", "standard") else 0.0
                score = min(0.8 + category_boost, 1.0)

                content = f"[{entry.category}] {entry.key}: {entry.value}"
                if entry.description:
...
class EpisodicMemoryRetriever(MemoryRetriever):
    ...
    def retrieve(self, query: RetrievalQuery) -> List[RetrievalResult]:
        ...
        try:
            results = []
            # Use episodic search with time/context filters
            ...
            for event in events:
                # Score based on outcome (success > partial > neutral > failure)
                outcome_scores = {"success": 0.9, "partial": 0.7, "neutral": 0.5, "failure": 0.3}
                score = outcome_scores.get(event.outcome, 0.5)

                # Boost if matches task context
                if query.context and query.context.get("task_id") == event.task_id:
                    score = min(score + 0.2, 1.0)

                content = f"[{event.event_type}] {event.title}"
                if event.description:
                    content += f": {event.description}"
                if event.outcome != "neutral":
                    content += f" (outcome: {event.outcome})"
...
class SemanticMemoryRetriever(MemoryRetriever):
    ...
    def retrieve(self, query: RetrievalQuery) -> List[RetrievalResult]:
        ...
        try:
            ...
            # Search semantic memory
            entries = self.memory.search(
                query.query if query.query else None,
                category=None,
                language=query.context.get("language") if query.context else None,
                tags=None,
                min_confidence=0.5,
